package com.ddlabs.atlassian.controller;
...
public class RestController {
    ...
    @Inject
    public RestController(MetricServerFactory metricServerFactory,
                          UserService userService, HttpClient httpClient, LocalStorageService localStorageService) {
    ...
    public Response request_accessToken(@Context HttpServletRequest req) {
        ...
        metricServer.saveServerMetadata(extractServerType(req.getParameter("domain")),
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.TEXT_PLAIN)
    public Response testConnection(APIAuthenticationType APIAuthenticationType, @Context HttpServletRequest req) {
        userService.isAuthenticatedUserAndAdmin();
        ValidationUtils.validateNotEmpty(APIAuthenticationType.getApiKey(), "API Key can not be empty");
        ValidationUtils.validateNotEmpty(APIAuthenticationType.getAppKey(), "App Key can not be empty");
        ValidationUtils.validateNotEmpty(APIAuthenticationType.getEndpoint(), "Endpoint can not be empty");
        String result = httpClient.get(APIAuthenticationType);
        if(!result.isEmpty()){
    ...
    @GET
    @Path("retry/submission")
    public Response retrySendingMetrics(@Context HttpServletRequest req) {
        localStorageService.retry();
